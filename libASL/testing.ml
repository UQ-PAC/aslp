...
let pp_enc_fields (f: fields): string =
  pp_list (fun (f,i) -> pp_instr_field f ^ "=" ^ string_of_int i) f

let pp_enc_list (encs: (fields * int) list): string =
  String.concat "\n"
    (List.map (pp_pair pp_enc_fields hex_of_int) encs)
...
let op_dis (env: Env.t) (dis: Dis.Env.t) (iset: string) (op: Primops.bitvector): stmt list opresult =
  let env = Env.copy env in
  let decoder = Eval.Env.getDecoder env (Ident iset) in
  try
    let stmts = Dis.eval_decode_case Unknown dis decoder (VBits (Left op)) in
    Result.Ok stmts
  with
...
let initializeRegisters (env: Env.t) (xs: Primops.bigint list): unit =
    let d = VBits {n=64; v=Z.zero} in
    let vals = List.mapi (fun i v -> (i, VBits {n=64; v})) xs in
    let arr = List.fold_left
        (fun a (k,v) -> Primops.ImmutableArray.add k v a)
        Primops.ImmutableArray.empty
        vals
    in
    Env.setVar Unknown env (Ident "_R") (VArray (arr, d))

let op_test_opcode (env: Env.t) (dis: Dis.Env.t) (iset: string) (op: int): Env.t opresult =
  let op = Primops.prim_cvt_int_bits (Z.of_int 32) (Z.of_int op) in

  let initenv = Env.copy env in
  Random.self_init ();
  let vals = (List.init 64 (fun _ -> Z.of_int64 (Random.int64 Int64.max_int))) in
  initializeRegisters initenv vals;
  initializeGlobals initenv;

  let (let*) = Result.bind in
  let* evalenv = op_eval initenv iset (VBits op) in
  let* disstmts = op_dis env dis iset op in
